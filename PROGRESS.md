...
本家サブスクBoxのPro機能「リアルタイム為替レート換算」に相当。円以外（USD/EUR）の
サブスクも登録でき、合計は基準通貨に換算して表示する。

- サブスクに**通貨**（JPY/USD/EUR）を追加（未設定の既存データは JPY 互換）
- 為替レートを**無料API（open.er-api.com, USD基準）からリアルタイム取得**
  - `localStorage`（`subsuku-box:rates`）に24時間キャッシュ
  - 取得失敗・オフライン時は**概算フォールバックレート**で継続
- サマリーカードに **JPY / USD / EUR の基準通貨切り替えトグル**を追加
- 一覧では各サブスクを元通貨で表示し、外貨は**円換算の目安**を併記
- 並び替え「月額が高い順」は通貨をまたぐため**円換算で比較**

**追加ファイル**
```
lib/currency.ts             通貨の整形・換算ユーティリティ（formatMoney/convert）
lib/useExchangeRates.ts     為替レート取得＋キャッシュのフック
```
...
円に加え USD / EUR を登録でき、合計は基準通貨に換算。為替レートは無料APIから
リアルタイム取得（キャッシュ＋フォールバックあり）。詳細は「完了した機能 → 7.」を参照。

### 🎯 次の候補

下の「その他の残課題 / 拡張アイデア」から、データエクスポート（JSON）や通知が着手しやすい。

---

## その他の残課題 / 拡張アイデア

- [x] **多通貨対応** — USD / EUR など円以外の通貨表示（リアルタイム為替換算つき）
...
├── lib/
│   ├── currency.ts           ← 通貨の整形・換算ユーティリティ
│   ├── format.ts
│   ├── templates.ts
│   ├── types.ts
│   ├── useExchangeRates.ts   ← 為替レート取得フック
│   ├── useSubscriptions.ts
